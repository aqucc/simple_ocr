...
- **一覧**: 保存日ごと（新しい日付が上）にサムネイル・テキスト・時刻・「コピー」・「再読み取り」・「削除」（`window.confirm` で確認）。**サムネイルをタップするとライトボックス（全画面オーバーレイ）で拡大表示**され、どこかをタップすると閉じます。「再読み取り」は保存済みの画像を撮影フローに読み込み直し、範囲選択からやり直せます（保存画像は最長800pxに縮小したJPEGのため、元の撮影解像度ではなくその解像度での再認識になります）。
