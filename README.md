...
7. **ノイズ耐性テスト（ハルシネーション対策の受け入れゲート）**: 斜めのグレーグラデーション背景＋約800個の低コントラストな砂嵰状ノイズ（点・短い線）＋軽いビネットの上に同じラベル文字を描画した画像（決定論的シード付き乱数で再現可能）を、切り抜き→英数字モード（PaddleOCR）と切り抜き→日本語モード（Tesseract、局所適応二値化＋デスペックルを経由）の両方に流し、期待文字列の正規表現一致に加えて「期待される文字列に含まれない英数字・CJK文字数（＝ハルシネーションの目安）」が Paddle は3文字以下、Tesseract は6文字以下であることを検証します。
